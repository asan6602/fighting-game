class Sprite {
    constructor({position, imageSrc, scale = 1, frames =1, offset = {x:0, y:0}}) {  //wrap position and velocity into one object
...
class Fighter extends Sprite{
    ...
    update() {
        ...
        if(this.position.y + this.height + this.velocity.y >= canvas.height - 62) {
           this.velocity.y = 0 //stops from being pulled all the way down
    ...
    switchSprite(sprite) {
        if (this.image === this.sprites.attack1.image && this.framesCurrent < this.sprites.attack1.frames - 1 ) {
            return
        }
        switch (sprite) {
            case'idle':
